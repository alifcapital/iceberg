...
| `delete-files-only` | false | If true, only consider files with delete files for rewriting. File size thresholds (`min-file-size-bytes` and `max-file-size-bytes`) are ignored. |
